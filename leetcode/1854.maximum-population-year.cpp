...
public:
    int maximumPopulation(vector<vector<int>>& logs) {
        int n = logs.size();
        int currPopulation=0, maxPopulation=0;
        int ans=1950;
 ...
 * deathi] indicates the birth and death years of the i^th person.
 *
 * The population of some year x is the number of people alive during
 * that year. The i^th person is counted in year x's population if x is
 * in the inclusive range [birthi, deathi - 1]. Note that the person is
 * not counted in the year that they die.
 *
