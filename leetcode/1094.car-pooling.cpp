...
public:
    bool carPooling(vector<vector<int>>& trips, int capacity) {
        int n=trips.size();
        int ans=0;
        vector<int> differenceArray(1001, 0);
        for(auto trip: trips){
            differenceArray[trip[1]] += trip[0];
            differenceArray[trip[2]+1] -= trip[0];
        }

        for(int i=1; i<differenceArray.size(); i++)
            differenceArray[i] += differenceArray[i-1];

        return (*max_element(differenceArray.begin(), differenceArray.end())) <= capacity;
    }
};
 ...
 * = [numPassengersi, fromi, toi] indicates that the i^th trip has
