...
class Solution {
public:
    bool valid(vector<int>& nums, int day, int m, int k){
        int n = nums.size();
        int len = 0, ans=0;
        for(int i=0;i<n;i++){
            if(nums[i] <= day){
    ...
    int minDays(vector<int>& nums, int m, int k) {
        int n = nums.size();
        if(m > (n/k)) return -1;
        int low = *min_element(nums.begin(), nums.end()), high = *max_element(nums.begin(), nums.end()), mid;
        int ans = high;
        while(low<=high){
            mid = midpoint(low, high);
            if(valid(nums, mid, m, k)){
                ans = min(ans, mid);
                high = mid -1;
            }else{
                low = mid+1;
 ...
 * Output: 3
 * Explanation: Let us see what happened in the first three days. x
 * means flower bloomed and _ means flower did not bloom in the garden.
 ...
 * need 6 flowers. We only have 5 flowers so it is impossible to get the
 ...
 * We can make one bouquet of the first three flowers that bloomed. We
 * cannot make another bouquet from the last three flowers that bloomed
