...
public:
    ...
    NumMatrix(vector<vector<int>>& matrix) {
        ...
        prefixMatrix.assign(n+1, vector<int>(m+1, 0));

        for(int i=1; i<=n; i++){
            vector<int> rowPrefix(m+1, 0);
            for(int j=1; j<=m; j++){
                rowPrefix[j] = rowPrefix[j-1] + matrix[i-1][j-1];
                prefixMatrix[i][j] = prefixMatrix[i-1][j] + rowPrefix[j];
            }
        }
    }
