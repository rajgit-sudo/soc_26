 ...
 * ──────────────────────────────────────────────────
 *
 * There is an m x n grid, where (0, 0) is the top-left cell and (m - 1,
 * n - 1) is the bottom-right cell. You are given an integer array
 * startPos where startPos = [startrow, startcol] indicates that
 ...
 * 0-indexed integer arrays: rowCosts of length m and colCosts of length
 * n.
 *
 * • If the robot moves up or down into a cell whose row is r, then
 * this move costs rowCosts[r].
 *
 * • If the robot moves left or right into a cell whose column is c,
 ...
 * Example 2:
 *
 * Input: startPos = [0, 0], homePos = [0, 0], rowCosts = [5], colCosts
 * = [26]
 * Output: 0
